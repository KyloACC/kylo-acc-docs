...
for i, stint in enumerate(stints):
    ...
    rolling_mean = filtered_data["laptime_in_ms"].rolling(window=5, center=True, min_periods=1).mean()
    ax.plot(filtered_data["lap"], rolling_mean/1000, color=colors[i], label=f"Stint {stint} ({len(filtered_data)} laps) - {driver}")
...
plt.title(sys.argv[4] + "- Moving Average")

# Display the plot
plt.show()
plt.savefig(sys.argv[2] + "_linechart_movingaverage.png", dpi=500)
